...
static enum bt_mesh_walk mod_sub_clear_visitor(struct bt_mesh_model *mod, uint32_t depth,
					       void *user_data)
{
	if (IS_ENABLED(CONFIG_BT_MESH_LOW_POWER)) {
			     ...
			     struct net_buf_simple *buf)
{
	...
	if (ARRAY_SIZE(mod->groups) > 0) {
		bt_mesh_model_graph_walk(mod, mod_sub_clear_visitor, NULL);
...
static enum bt_mesh_walk mod_sub_list_visitor(struct bt_mesh_model *mod, uint32_t depth, void *ctx)
{
		       ...
		       struct net_buf_simple *buf)
{
	...
	bt_mesh_model_graph_walk(mod, mod_sub_list_visitor, &visit_ctx);
